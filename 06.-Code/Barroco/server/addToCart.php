...
$conn->query("UPDATE shopping_cart 
              SET total = (SELECT IFNULL(SUM(subtotal), 0) FROM cart_item WHERE idShoppingCart = $cartId)
              WHERE idShoppingCart = $cartId");

$conn->query("UPDATE `order` 
              SET total = (SELECT total FROM shopping_cart WHERE idShoppingCart = $cartId)
              WHERE idOrder = (SELECT idOrder FROM shopping_cart WHERE idShoppingCart = $cartId)");
